...
const Site2 = use('App/Models/Site2')
const Env = use('Env')
...
class Site1Controller {
  ...
  async getData({request}){    
    ...
    const site1 = await Site2.query().paginate(page,limit)
  ...
  async index ({request}) {
    const site1 = await Site2.all()
    const gethujan = await Site2.query().getAvg('hujan')
    const hujan =  parseFloat(gethujan).toFixed(1)
    const getsuhu = await Site2.query().getAvg('suhu')
    const suhu = parseFloat(getsuhu).toFixed(1)
    const getpasut = await Site2.query().getAvg('pasut')
    const pasut = parseFloat(getpasut).toFixed(1)

    return {site1, hujan, suhu, pasut}
  }

module.exports = Site2Controller